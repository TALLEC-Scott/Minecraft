...
void Menu::drawDirtBackground(UIRenderer& ui, int windowW, int windowH) {
    ...
    // Slow enough to feel ambient but fast enough that NEAREST sampling
    // advances ~1 texel every few frames instead of snapping once a second.
    // Reads as intentional pixel-art stepping rather than low framerate.
    float scroll = (float)glfwGetTime() * 0.3f;
